...
core.event("org.bukkit.event.block.BlockPlaceEvent",(evt)=>{
    ...
    if(evt.getBlockPlaced().getBlockData().getMaterial().name() == "END_PORTAL_FRAME"){
        function check(nx,nz) {
            if (bloco.getWorld().getBlockAt(bloco.getX() + nx, bloco.getY(), bloco.getZ() + nz).getBlockData().getMaterial().name() == "AIR") return true
            else return false
        }
        ...
        function criarportal(){
            ...
            addframe(1,0); addframe(-1,0); addframe(1,1); addframe(-1,1);
            addframe(0,1); addframe(0,-1); addframe(1,-1); addframe(-1,-1);
